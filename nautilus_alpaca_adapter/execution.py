...
from nautilus_alpaca_adapter.core import ALPACA_VENUE
from nautilus_alpaca_adapter.error import AlpacaError
from nautilus_alpaca_adapter.http import AlpacaHttpClient
from nautilus_alpaca_adapter.parser import (
    parse_order_status,
    parse_order_status_http,
    parse_order_type,
    parse_position_report,
    parse_trade_report,
)
from nautilus_alpaca_adapter.providers import AlpacaInstrumentProvider
from nautilus_alpaca_adapter.websocket import AlpacaWebSocketClient
...
class AlpacaExecutionClient(LiveExecutionClient):
    ...
    async def _update_account_state(self) -> None:
        ...
        if self.account_id is None:
            self._set_account_id(
                AccountId(f"{ALPACA_VENUE.value}-{response['account_number']}")
            )
    ...
    async def generate_order_status_reports(
        ...
        active_symbols: Set[str] = {o.instrument_id.symbol.value for o in open_orders}
        ...
        try:
            open_order_msgs: List[
                Dict[str, Any]
            ] = await self._http_client.get_open_orders(
            ...
            for symbol in active_symbols:
                response = await self._http_client.get_order_history(
                    market=symbol,
                    after=secs_to_millis(start.timestamp())
    ...
    async def _submit_order(self, order: Order, position: Optional[Position]) -> None:
        ...
        if order.order_type in {OrderType.MARKET, OrderType.LIMIT}:
            ...
            kargs.update(type="stop", trigger_price=str(order.trigger_price))
            ...
            kargs.update(
                ...
                if order.is_buy
                else str(-order.trailing_offset),
            )
    ...
    async def _cancel_order(self, command: CancelOrder) -> None:
        ...
        try:
            if command.venue_order_id is not None:
                ...
                await self._http_client.cancel_order_by_client_id(
    ...
    def _generate_external_order_report(
        ...
        report = OrderStatusReport(
            ...
            price=instrument.make_price(data["limit_price"])
            if data["limit_price"]
            else None,
            trigger_price=instrument.make_price(data["stop_price"])
            if data["stop_price"]
            else None,
            ...
            avg_px=Decimal(data["filled_avg_price"])
