...
from nautilus_trader.model.instruments.equity import Decimal, Equity
...
class AlpacaInstrumentProvider(InstrumentProvider):
        ...
        for data in assets:
            if data["tradable"]:
                self._parse_instrument(
                    data, account_info["currency"], account_info["multiplier"]
                )
    ...
    async def load_ids_async(
        ...
        for instrument_id in instrument_ids:
            PyCondition.equal(
                instrument_id.venue, self.venue, "instrument_id.venue", "self.venue"
            )

            data: Dict[str, Any] = await self._http_client.get_market(
    ...
    async def load_async(
        self, instrument_id: InstrumentId, filters: Optional[Dict] = None
    ):
        if filters is not None:
            raise NotImplementedError("Filter feature must be implemented.")
