...
from nautilus_alpaca_adapter.parser import parse_bar_http, parse_quote_ticks, parse_trade_ticks
...
class AlpacaDataClient(LiveMarketDataClient):
    ...
    def subscribe_ticker(self, instrument_id: InstrumentId) -> None:
        self._log.error("Cannot subscribe to ticker data (not supported by Alpaca).")

    def subscribe_quote_ticks(self, instrument_id: InstrumentId) -> None:
        self._loop.create_task(
            self._ws_client.subscribe_quotes(instrument_id.symbol.value)
        )
    ...
    def unsubscribe_ticker(self, instrument_id: InstrumentId) -> None:
        self._log.error("Cannot unsubscribe from ticker data (not supported by Alpaca).")

    def unsubscribe_quote_ticks(self, instrument_id: InstrumentId) -> None:
        self._loop.create_task(
            self._ws_client.unsubscribe_quotes(instrument_id.symbol.value)
        )

    def unsubscribe_trade_ticks(self, instrument_id: InstrumentId) -> None:
        self._loop.create_task(
            self._ws_client.unsubscribe_trades(instrument_id.symbol.value)
        )
    ...
    async def _request_quote_ticks(
        ...
        data = await self._http_client.get_quote_ticks(
            instrument_id.symbol.value, from_datetime, to_datetime, limit
        )
